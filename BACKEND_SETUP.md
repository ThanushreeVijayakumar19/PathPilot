...
## 5. Real internship listings (Adzuna) — optional but recommended

Recommendations pull real, current internship listings from Adzuna's job
search API when configured. Without it, the app falls back to AI-generated
practice listings automatically — nothing breaks either way.

1. Go to **developer.adzuna.com/signup** and create a free account (instant,
   no credit card).
...
4. Restart `npm run dev`. Recommendations will now show real listings
   (marked "Verified listing") mixed with any AI-generated ones needed to
   round out the batch.

Note: real job platforms don't have a formal "externship" category — those
are best-effort classified from listing wording, not guaranteed.
